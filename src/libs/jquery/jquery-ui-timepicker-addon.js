...
(function($) {
	if ($.ui.timepicker = $.ui.timepicker || {}, !$.ui.timepicker.version) {
		...
		$.extend(Timepicker.prototype, {
			...
			datetimepickerSmall: function() { // by songxj
				/**
				 * [getValue 获取时间的函数]
				 * @author huzc
				 * @date   2015-07-09
				...
				var bindEvents = function($input, hour, minute, second) {
					...
					jQuery.getScript('/libs/jquery/jquery-rollValue.js', function(data, textStatus) {
						if (textStatus === "success") {
							jQuery(".datetime-picker-small-panel").find("input.mydate").each(function(k) {
				...
				this.each(function() {
					...
					$input.off("focus").on("focus", function() {
						...
						var html = [
							...
							'<span class="item"><input type="number1" maxLength="2" min="1" max="31" step="1" class="date mydate mousewheelnum" value="' + date[2] + '" minValue="1" readonly1 />日</span>',
							'</div>',
		...
		$.timepicker = new Timepicker, $.timepicker.timeZoneOffsetString = function(e) {
			...
			window.consoles && consoles.log(e)
